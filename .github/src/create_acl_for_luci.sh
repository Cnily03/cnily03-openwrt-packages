...
error_font="\033[1;31m[Error]$\033[0m "
error_font="\033[1;31m[Error]$\033[0m "
success_font="\033[1;32m[Success]\033[0m "
info_font="\033[1;36m[Info]\033[0m "
...
function auto_create_acl(){
	...
	do
		if check_if_acl_exist "$i"; then
			echo_yellow "$i: has ACL file already, skipping..." | tee -a create_acl_for_luci.warn
		elif check_config_files "$i"; then
			echo_red "$i: has no/multi config file(s), skipping..." | tee -a create_acl_for_luci.err
		else
			create_acl_file "$i/root/usr/share/rpcd/acl.d" "${i##*/}" "$(get_config_name "$i")"
			echo_green "$i: ACL file has been generated." | tee -a create_acl_for_luci.ok
		fi
	done
}
...
if [ "*${manual_mode}*" == "*1*" ] && [ -n "${luci_name}" ] && [ -n "${conf_name}" ]; then
	...
	if create_acl_file "${acl_path}" "${luci_name}" "${conf_name}"; then
		...
		echo_green "$(cat "${acl_path}/${luci_name}.json")"
		echo_green "${luci_name}: ACL file has been generated." >> "create_acl_for_luci.ok"
		...
		echo_red "${luci_name}: Failed to create ACL file." >> "create_acl_for_luci.err"
	fi
else
	echo -e "${info_font}Usage: $0 [-a|-m -p <path-to-acl> -l <luci-name> -n <conf-name>|-c]"
	exit 2
